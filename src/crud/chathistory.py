...
def create_chat_message(db: Session, employee_id: int, question: str, response: str):
    ...
    if not employee:
        raise HTTPException(
            ...
            detail=f"Employee '{employee_id}' is Not Found",
        )
...
def get(db: Session, employee_id: int) -> List[dict]:
    employee_history = (
        db.query(ChatHistory).filter(ChatHistory.employee_id == employee_id).all()
    )
    ...
    if not employee_history:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No History available for '{employee_id}'",
        )
    ...
    for emp in employee_history:
        ...
        if isinstance(emp.timestamp, str):
            history_create_dt = datetime.fromisoformat(
                emp.timestamp
        ...
        if isinstance(emp.expiration_timestamp, str):
            ...
            raise ValueError(
                f"Unexpected expiration timestamp type: {type(emp.expiration_timestamp)}"
            )
