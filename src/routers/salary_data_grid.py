...
from src.models.salary_models import Employee, Department, Position, Incentive, TableMetadata, ColumnMetadata

router = APIRouter(tags=["salary-data"])

@router.get("/salary-data")
def get_salary_data(table: str = "employee_performance", db: Session = Depends(get_salary_db)):
    """
    ...
    if not table_meta:
        ...
        raise HTTPException(status_code=404, detail=f"Table metadata not found for '{table}'")

    # 2. Fetch Column Metadata
    # Sorted by id (insertion order)
    columns_meta = db.query(ColumnMetadata).filter_by(table_id=table_meta.id).order_by(ColumnMetadata.id).all()
