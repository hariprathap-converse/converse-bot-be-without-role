...
async def create_history(
    ...
    if not data:
        raise HTTPException(
            ...
            detail="there is Error in Store History",
        )
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail="History Created Successfully"
    )


@router.get(
    "/history", dependencies=[Depends(roles_required("admin", "employee", "teamlead"))]
)
