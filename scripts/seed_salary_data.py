...
from src.core.salary_database import SalaryBase, SalarySessionLocal, salary_engine
        ...
        if db.query(Employee).count() > 0:
            ...
            for row in raw_data:
                ...
                if dept_name not in departments:
                    colors = department_colors.get(
                        dept_name,
                        {"bg": "#ffffff", "text": "#000000", "border": "#cccccc"},
                    )
                    dept = Department(
                        name=dept_name,
                        color_bg=colors["bg"],
            ...
            for row in raw_data:
                ...
                for _ in range(num_incentives):
                    ...
                    incentive = Incentive(employee_id=emp.id, amount=amount, date=date)
                    db.add(incentive)
        ...
        if db.query(TableMetadata).filter_by(table_name="employee_performance").first():
            ...
            columns_data = [
                ...
                },
                {
                    ...
                    "cellConfig": {
                        "colorMap": "dynamic"
                    },  # Dynamic map will be resolved in API
                },
                {
